  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-sidebar text-sidebar-foreground h-full flex w-(--sidebar-width) flex-col",
          className
        )}
        {...props}
      >
...
function SidebarMenuButton({
  ...
  const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    // trigger user-defined click behavior if any
    onClick?.(event);

    // Auto-close sidebar on click if mobile
    if (isMobile) {
      setOpenMobile(false);
    } else {
      setOpen(true);
